	...
	private static String removeCharsInString(String text, char start, char end) {
		...
		if (text.length() > 0) {
			...
			if (ch > start && ch < end) {
				...
				return "" + removeCharsInString(text.substring(0, text.length() - 1), start, end) + ch;
			}
		}
		return "";
	}
	...
	private static String shiftDigitRight(String text) {
		...
		if (text.length() > 0) {
			if (Character.isDigit(text.charAt(0))) {
				return shiftDigitRight(text.substring(1)) + text.charAt(0);
			} else {
				return text.charAt(0) + shiftDigitRight(text.substring(1));
			}
		}
		return "";
	}
